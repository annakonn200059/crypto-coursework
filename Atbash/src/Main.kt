...
fun transformKey(key: String, alphabet: String): IntArray {
    return key.map { alphabet.indexOf(it) }.toIntArray()
}

fun transform(text:String, alphabet:String, oldalphabet:String): String {
var s =""
    val l = text.length-1
  for (i in 0..l){
...
fun deletelettersfromalphabet(alp:String, key1:IntArray, key2:String):String{
    var k=alp
...
fun wordwithoutkey(text:String, oldalph: String,alph:String):String{//функция для расшифровки
    var s =""
    val l = text.length-1
    for (i in 0..l){
        if (text[i].toUpperCase() in alph || text[i].toUpperCase() in oldalph) {
            val k = oldalph.indexOf(text[i].toUpperCase())
            val o = oldalph.indexOf(oldalph[k].toUpperCase())
...
fun main() {
    ...
    val openTextElement = document.getElementById("open-text") as HTMLTextAreaElement
    val cipherTextElement = document.getElementById("cipher-text") as HTMLTextAreaElement
    ...
    (document.getElementById("encrypt-btn") as HTMLButtonElement).onclick = {
        ...
        if ((k_letters + k_symbols) != word.length) {
            ...
            val verynewalph = newalp.reversed()
    ...
    (document.getElementById("decrypt-btn") as HTMLButtonElement).onclick = {
        val pushedel = document.getElementById("lang") as HTMLSelectElement
        val sellang = pushedel.value
        val alphabet = if (sellang=="eng") {alphabetENG } else alphabetRUS
